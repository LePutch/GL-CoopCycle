...
type RestOf<T extends ICommande | NewCommande> = Omit<T, 'dateTime'> & {
...
export class CommandeService {
  ...
  protected convertDateFromClient<T extends ICommande | NewCommande | PartialUpdateCommande>(commande: T): RestOf<T> {
    return {
      ...commande,
      dateTime: commande.dateTime?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restCommande: RestCommande): ICommande {
    return {
      ...restCommande,
      dateTime: restCommande.dateTime ? dayjs(restCommande.dateTime) : undefined,
    };
  }
